...
class PlayState extends MusicBeatState
{
	...
	private var isPoison:Bool = false;
	private var poison:Float = 0.0025;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			case 'cyberBG':
			{
				...
				var cbg:FlxSprite = new FlxSprite(-93.4, -407.05).loadGraphic(Paths.image('bg', 'tutorial'));
				cbg.scrollFactor.set(1, 1);
				add(cbg);

				var clights:FlxSprite = new FlxSprite(-103.3, -191.9);
				clights.frames = Paths.getSparrowAtlas('wires', 'tutorial');
				clights.animation.addByPrefix('idle', "Ls", 24);
				clights.animation.play('idle');
				clights.scrollFactor.set(1, 1);
				add(clights);

				var cfloor:FlxSprite = new FlxSprite(-93.4, 373.1).loadGraphic(Paths.image('floor', 'tutorial'));
				cfloor.scrollFactor.set(1, 1);
				add(cfloor);

				var clight:FlxSprite = new FlxSprite(-72.3, 834.25).loadGraphic(Paths.image('lights', 'tutorial'));
				clight.scrollFactor.set(1, 1);
				add(clight);
			}
			case 'queenBG':
			{
				...
				var qbg:FlxSprite = new FlxSprite(-341.05, -235.55).loadGraphic(Paths.image('queenBG/bg', 'queen'));
				qbg.scrollFactor.set(0.1, 0.1);
				add(qbg);

				var qlights:FlxSprite = new FlxSprite(-299.9, -117.25);
				qlights.frames = Paths.getSparrowAtlas('queenBG/town', 'queen');
				qlights.animation.addByPrefix('idle', "city", 24);
				qlights.animation.play('idle');
				qlights.scrollFactor.set(0.2, 0.2);
				add(qlights);

				var fountain:FlxSprite = new FlxSprite(224.55, -235.15);
				fountain.frames = Paths.getSparrowAtlas('queenBG/vulcan', 'queen');
				fountain.animation.addByPrefix('idle', 'animated fountain', 24);
				fountain.animation.play('idle');
				fountain.scrollFactor.set(0.6, 0.6);
				add(fountain);

				var qfloor:FlxSprite = new FlxSprite(-341.05, 421.1).loadGraphic(Paths.image('queenBG/floor', 'queen'));
				qfloor.scrollFactor.set(1, 1);
				add(qfloor);
			}
			case 'stage':
			{
		...
		{
			case 'gf':
				healthBar.createFilledBar(0xFFB3103B, 0xFFB752FF);
			case 'queen':
				healthBar.createFilledBar(0xFF3870FF, 0xFFB752FF);
		}
		// healthBar
		add(healthBar);

		// Add Kade Engine watermark
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | KE " + 1.69 : ""), 16);
	...
	override public function update(elapsed:Float)
	{
		...
		if (dad.curCharacter == "queen")
		{
			dad.y += Math.sin(floatvalve);
			dad.y += Math.cos(floatvalve);
		}

		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, 0.70)));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, 0.70)));
	...
	function noteMiss(direction:Int = 1, daNote:Note = null):Void
	{
		if (!boyfriend.stunned)
		{
			...
			FlxG.sound.play(Paths.sound('damage'), 0.7); //, FlxG.random.float(0.1, 0.2));
			// FlxG.sound.play(Paths.sound('missnote1'), 1, false);
			// FlxG.log.add('played imss note');

			//custom notes shit
			if(daNote.noteStyle == 'banana')
			{
				FlxG.sound.play(Paths.sound('error'));
				isPoison = true;
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'palace-raid')
		{
			if (curBeat == 1 && !songEnded)
			{
				FlxTween.tween(camHUD, {alpha: 0}, 0.5, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
					{
						camHUD.visible = false;
					}
				});
			}
			if (curBeat == 23 && !songEnded)
			{
				FlxTween.tween(camHUD, {alpha: 1}, 0.3, {onComplete: function(twn:FlxTween)
					{
						camHUD.visible = true;
					}
				});
